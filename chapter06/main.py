# 主要包含四个函数，其中三个需要命令行执行-->fire
# train()
# test()
# val():计算模型在验证集上的准确率，辅助训练
# help():log
...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for ii,(data,label) in tqdm(enumerate(train_dataloader)):
            ...
            confusion_matrix(score.detach(),target.detach())
...
def val(model, dataloader):
    ...
    计算模型在验证集上的准确率等信息
    """
    model.eval() # 测试过程中要保证BN层的均值和方差不变;不进行随机舍弃神经元
    confusion_matrix = meter.ConfusionMeter(2)
